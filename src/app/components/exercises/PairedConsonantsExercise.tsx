...
const questions = [
  {
    ...
    options: ['четверк', 'четверг'],
    checkWord: 'четверга',
    explanation: 'Проверочное слово: четверга',
  },
  {
    word: 'сне_',
    correctAnswer: 'снег',
  ...
  },
  {
    ...
    options: ['глас', 'глаз'],
    checkWord: 'глаза',
    explanation: 'Проверочное слово: глаза',
  },
  {
    word: 'мё_',
    correctAnswer: 'мёд',
    options: ['мёт', 'мёд'],
    checkWord: 'мёда',
    explanation: 'Проверочное слово: мёда',
  },
];

interface PairedConsonantsExerciseProps {
  onComplete: (questId: string, bones: number) => void;
}

export function PairedConsonantsExercise({ onComplete }: PairedConsonantsExerciseProps) {
  ...
  const checkAnswer = () => {
    ...
    setTimeout(() => {
      if (currentIndex < questions.length - 1) {
        ...
        onComplete('paired-consonants', bones);
      }
    }, 3500);
  };
      ...
      {!showResult ? (
        <div className="space-y-8">
          <div className="text-center py-8">
            <p className="text-4xl font-light text-gray-900 tracking-wide">
